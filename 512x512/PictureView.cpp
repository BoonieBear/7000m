...
//	if(m_bIsDraw == TRUE)		//只有点击选择按钮，才进行重绘
	{
		m_bIsDraw = pDib.DrawBig( pdc, 5, 5, 1);
...
//	imageCode.ColorCode(BMPBuffer,(BYTE *)databuff->buf,256);
	CString jpcstr;
	jpcstr.Format("%s",filename);
	jpcstr += ".jpc";
	char* opt[11] = {"ACS.exe","-f",filename,"-t","bmp","-T","jpc","-F",jpcstr.GetBuffer(0),"-O","rate=0.021"};
  	int ret = Jpeg2000_Coder(11,opt);
	jas_cleanup();
		
	if (!ret)
	{
		AfxMessageBox("图像编码出错！！！");
		return;
	}
	DIGITALBUFF *databuff;
	databuff = (DIGITALBUFF *)malloc(sizeof(DIGITALBUFF));
	databuff->len = IMAGESIZE512X512;
	databuff->buf = (unsigned char *)malloc(databuff->len);
	FILE *fp;	
	fp=fopen(jpcstr,"rb");
	fread(databuff->buf,IMAGESIZE512X512*sizeof(BYTE),1,fp);
	fclose(fp);

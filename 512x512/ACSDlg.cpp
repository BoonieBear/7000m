...
unsigned char msg[240*70]={0};
unsigned char code[240*84]={0};
unsigned char decode[240*70]={0};
unsigned char rx[240*84]={0};
unsigned char err_pos[14]={0};//错误位置取值0~83，不能有相同的项
//////////////////////////////////////////////////////////////////////////
#ifdef _DEBUG
...
BOOL FskWrong=false;//fsk所在帧错误标志
BOOL FskParseOk=false;//fsk所在帧已处理
...
BEGIN_MESSAGE_MAP(CACSDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_TESTPIC, OnTestpic)
	ON_WM_NCPAINT( )
	...
	ON_BN_CLICKED(IDC_SHOW, OnImage)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
//	m_tabctrl.SetCurSel(0);
	...
	pShowPic.Create(IDD_DIALOG_SHOWPIC,this);
	pShowPic.ShowWindow(SW_HIDE);
	//LPt设置
...
void CACSDlg::OnPaint() 
{
	if (IsIconic())
	{
		...
		{	
			CString str = "系统正在录音……";
			dc.DrawText(str,rect,DT_SINGLELINE|DT_CENTER);
		}
	}
	else
	{
...
//						PackageShipData((unsigned char *)buf+32,len-32);
				...
				else if(UWACInitParam.XMTModulation==MPSKMOD)//MPSK
				{
					if (!UWACInitParam.ADIOWorkMode)
					{
						SetTimer(LBLTIMER,UWACInitParam.SyncConcurrence*9*1000+2000,NULL);//开始通信周期计时
					}
					...
					memcpy(buf,(unsigned char *)&sendpacket,32);//拷贝网络数据头格式
					memset((unsigned char *)buf+32,0,len-32);//拷贝全零
					PackagePicTureIn512x512((unsigned char *)buf+32,len-32);
...
void CACSDlg::ExplainRecData(BYTE *databuf, int len, int ntype)
{
	...
	case MPSKMOD://PSK
		if (UWACInitParam.ADIOWorkMode&&bFirstRecvData)
		{
			...
			m_LED.Invalidate(TRUE);
			RECVMPSK = true;
			SetTimer(VOICEENABLE,8000*11,NULL);
		}
		bFirstRecvData = FALSE;
		ExpainPskAndFsk(databuf,len);
...
void CACSDlg::TestPicturePackageAndParse()
{
	int totlevaliddatabytelen =0;
	int i=0,j=0;
	totlevaliddatabytelen = UWACInitParam.XMTMPSKPackageNum*UWACInitParam.XMTMPSKPackFramNum
...
void CACSDlg::PackagePicTureIn512x512(unsigned char *buf, int len)
{
	int totlevaliddatabytelen =0;
	int i=0,j=0;
	totlevaliddatabytelen = UWACInitParam.XMTMPSKPackageNum*UWACInitParam.XMTMPSKPackFramNum
	...
	BYTE* acousticdata = new BYTE[totlevaliddatabytelen/2];//母船声学数据缓存
	BYTE* formerdata = new BYTE[IMAGESIZE512X512];//声学数据缓存
	BYTE Turbodata[242];//Turbo编码前数据缓存
	memset(acousticdata,0,(totlevaliddatabytelen/2)*sizeof(BYTE));
	memset(formerdata,0,IMAGESIZE512X512*sizeof(BYTE));
	memset(&Turbodata,0,bytelenperframe/2*sizeof(BYTE));
	...
	//////////////////////////前69帧数据数据///////////////////////////////////
	memset(msg,0,240*70*sizeof(char));
	memset(code,0,240*84*sizeof(char));
	memcpy(msg,formerdata,IMAGESIZE512X512);//rs编码前数据
	BYTE UWAData[UWAMFSKSIZE];
	memset(UWAData,0,UWAMFSKSIZE*sizeof(BYTE));
	/////////////////////////第70帧数据fsk////////////////////////////////////
	PackageUWADataInPsk(UWAData,UWAMFSKSIZE);
	memcpy(msg + IMAGESIZE512X512,UWAData,UWAMFSKSIZE);
	/////////////////////////RS编码///////////////////////////////////////////
	rs_enc(msg,code);//编码
	...
	for (i=0;i<UWACInitParam.XMTMPSKPackageNum*UWACInitParam.XMTMPSKPackFramNum;i++)//
	{
		memcpy(acousticdata+i*242,code+i*240,240*sizeof(char));
		nCRC16 = pDataPackage.CRC16(acousticdata+i*242,240);
		memcpy(acousticdata+i*242+240,(BYTE*)&nCRC16,sizeof(USHORT));
	}
	//////////////////////////////////////////////////////////////////////////
	...
	int m=0;
	XMTPSKFile.Init("rsPSK",4*FILESIZE,1024,4*WRITESIZE);
...
void CACSDlg::PackageUWADataInPsk(unsigned char *buf, int len)//在psk数据中插入的fsk数据
{
	int totlevaliddatabytelen =0;
	int i=0;
	mfskSUBPOSITION mfsksubposition;
...
//third part done!
	...
	//保存原数据文件
	XMTFSKFile.Init("subFSKinPSK",1*FILESIZE,1024,4*WRITESIZE);
	XMTFSKFile.SaveFile((char*)&acousticdata,UWAMFSKSIZE);
	XMTFSKFile.CloseFile();
	memcpy(buf,(char*)&acousticdata,UWAMFSKSIZE);
}

void CACSDlg::ExpainPskAndFsk(BYTE *databuf, int len)
{
	DWORD totlevaliddatabytelen;
	CString str;
	USHORT checksum16 = 0;
	...
	{// 正确帧
		...
		{//新一次数据接收的开始,进行清空操作
			str.Format("开始接收PSK信号，起始帧号%d。",frameno);
			SaveRecord(str);
			FskWrong=false;//fsk所在帧错误标志
		    FskParseOk=false;//fsk所在帧已处理
			iframenum = 0;
			ErrIndex = -1;
			memset(decode,0,240*70*sizeof(BYTE));
			memset(RecDataBuf,0,totlevaliddatabytelen);//缓冲区清空
			memset(rx,0,240*84*sizeof(unsigned char));
			RECVPSKFile.CloseFile();
			RECVPSKFile.Init("RECVPSK",FILESIZE,1024,4*WRITESIZE);
		...
		if (frameno==1&&RecFrameNo==UWACInitParam.RecvMPSKPackageNum*UWACInitParam.RecvMPSKPackFramNum-1)
		{
			str.Format("未收到第0帧数据!");
			SaveRecord(str);
			RECVPSKFile.CloseFile();
		...
		memcpy((BYTE*)&checksum16,(BYTE*)&RecDataBuf[(iframenum+1)*UWACInitParam.MPSKDataSymbolNum/8-2],sizeof(USHORT));//crc16
		USHORT CRC16 = pDataPackage.CRC16(RecDataBuf+iframenum*UWACInitParam.MPSKDataSymbolNum/8,240);
		memcpy(rx+iframenum*240,databuf+4,240);
		if (checksum16==CRC16)
		{
			if (frameno==69)
			{
...
//		imageDecode.ColorDecode(mybuffernew,CompressValue,256);
		////////////////////////////解码//////////////////////////////////////////
		int *FinalCodedbit = (int *)malloc(UWAMFSKSIZE*8*sizeof(int));
		BYTE acousticdata[UWAMFSKSIZE];//潜器声学数据缓存
		memset(acousticdata,0,UWAMFSKSIZE*sizeof(BYTE));
		if (ErrIndex>13)
		{
			SaveRecord("错误帧数过多，无法解析出数据！！");
			if(!FskWrong)//fsk正确
			{
				SaveRecord("收到潜器数据！");
				
				
				memcpy(acousticdata,RecDataBuf+242*69,UWAMFSKSIZE*sizeof(BYTE));
				//bit流
				for(int i=0;i<UWAMFSKSIZE;i++)
				{
					for(int j=0;j<8;j++)
					{
						FinalCodedbit[8*i+j] = (acousticdata[i]>>j)&0x01;
					}
				}
				RECVPSKFile.Init("subFSk",FILESIZE,1024,4*WRITESIZE);
				RECVPSKFile.SaveFile((char*)&acousticdata,UWAMFSKSIZE*sizeof(BYTE));
				RECVPSKFile.CloseFile();
				ParseUWAData(FinalCodedbit);
				FskParseOk = TRUE;
				free(FinalCodedbit);
			}
		...
		{
			rs_dec(rx,decode,err_pos,ErrIndex+1);//rs解码
			RECVPSKFile.Init("RS_DEC",FILESIZE,1024,4*WRITESIZE);
			RECVPSKFile.SaveFile((char*)&decode,240*70*sizeof(BYTE));
			RECVPSKFile.CloseFile();
			SaveRecord("完成对数据纠错!");
		...
		memcpy(CompressValue,decode,IMAGESIZE512X512);
		RECVPSKFile.Init("RECVJPC",FILESIZE,1024,4*WRITESIZE);
		RECVPSKFile.SaveFile((char*)&CompressValue,IMAGESIZE512X512*sizeof(BYTE));
		if (ErrIndex<=13)
		{
			CString jpcstr = RECVPSKFile.m_Path + RECVPSKFile.m_sFileName;
			RECVPSKFile.CloseFile();
			char filename[256];
			sprintf(filename,"%s",RECVPSKFile.m_Path+"RECV_IMAGE");
			CreateFileName(filename,".bmp",filename);
			char* decopt[9] = {"ACS.exe","-f",jpcstr.GetBuffer(0),"-t","jpc","-T","bmp","-F",filename};
			int ret = Jpeg2000_Coder(9,decopt);//解码
			if (!ret)
			{
				AfxMessageBox("图像解码错误！");
			}
			else
			{
				CRect rect;
				...
				pWnd->GetClientRect(&rect);
				m_ShowImage.InvalidateRect(&rect);
				m_ShowImage.FileLoad(filename);
				CTime nowtime;
				nowtime = CTime::GetCurrentTime();
				str.Format("%02d:%02d:%02d",nowtime.GetHour(),nowtime.GetMinute(),nowtime.GetSecond());
				SetDlgItemText(IDC_PIC_RECVTIME,str);
				pShowPic.AddToList(filename);
			}
		}
		//////////////////////////////////////////////////////////////////////////
		if (RecDataBuf)
...
void CACSDlg::OnTestpic() 
{
	// TODO: Add your control notification handler code here
	//package
	int totlevaliddatabytelen = UWACInitParam.XMTMPSKPackageNum*UWACInitParam.XMTMPSKPackFramNum
 		*UWACInitParam.MPSKDataSymbolNum*2/8;//可发送总字节数;
// 	BYTE* buf = (unsigned char *)malloc(totlevaliddatabytelen);
 	
// 	int i=0;
// 	memset((unsigned char *)buf,0,totlevaliddatabytelen);//拷贝全零
// 	PackagePicTureIn512x512((unsigned char *)buf,totlevaliddatabytelen);
	//parse
	BYTE data[242];
	FILE *fp = fopen("psk.dat","rb");
	for (int i =0;i<84;i++)
	{
		
		int ret = fread(&data[4],1,242,fp);
		*(DWORD*)data = i;
		ExpainPskAndFsk(data,246);
	}
}

void CACSDlg::OnImage() 
{
	// TODO: Add your control notification handler code here
	pShowPic.ShowWindow(SW_SHOW);
}

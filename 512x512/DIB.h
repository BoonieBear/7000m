...
public:
    ...
    virtual BOOL Load(CString pszFileName );// load DIB from disk file
	
    virtual BOOL Load(CFile *fp);               // load from file
    virtual BOOL Save(CString pszFileName);// save DIB to disk file
	...
	virtual void DrawInMiddle(CDC *pDC, int x, int y);
	virtual void DrawBig(CDC *pDC, int x, int y,CPoint p1,CPoint p2);
	virtual int DrawBig(CDC *pDC, int x, int y, int m);//增加放大倍数参数

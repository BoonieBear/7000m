...
BOOL CDIB::Load(CString pszFileName)
{
    CString strFile;    
...
BOOL CDIB::Save(CString pszFileName)
{
    CString strFile;    
...
void CDIB::DrawInMiddle(CDC *pDC, int x, int y)
{
	if((DibWidth()>512 ||DibHeight()>512))
	{
		MessageBox(NULL,"图像不符规格，请重新选择","注意",MB_ICONWARNING);
		return;
	}

    StretchDIBits(pDC->GetSafeHdc(),
                  x+(DibWidth()/4),                        // dest x
                  y+(DibHeight()/4),                        // dest y
                  DibWidth(),                 // dest width
                  DibHeight(),                // dest height
                  0,                        // src x
                  0,                        // src y
...
int CDIB::DrawBig(CDC *pDC, int x, int y,int m)
{
	if((DibWidth()>512 ||DibHeight()>512))
	{
		MessageBox(NULL,"图像不符规格，请重新选择","注意",MB_ICONWARNING);
    ...
    StretchDIBits(pDC->GetSafeHdc(),
                  x,                        // dest x
                  y,                        // dest y
                  DibWidth()*m,                 // dest width
                  DibHeight()*m,                // dest height
                  0,                        // src x
                  0,                        // src y
                  DibWidth(),                 // src width
                  DibHeight(),                // src height
...
void CDIB::DrawBig(CDC *pDC, int x, int y, CPoint p1,CPoint p2)
{


	StretchDIBits(pDC->GetSafeHdc(), //将整个图像放大
                  x,                        // dest x
                  y,                        // dest y
                  DibWidth()*2,                 // dest width
                  DibHeight()*2,                // dest height
                  0,                        // src x
                  0,                      // src y
                  DibWidth(),                // src width
                  DibHeight(),            // src height
 ...
 /*   StretchDIBits(pDC->GetSafeHdc(), //在原位置放大，图形大小不变，实际情况：将变得不清晰
                  x,                        // dest x
                  y,                        // dest y
                  DibWidth(),                 // dest width
                  DibHeight(),                // dest height
                  min(p1.x,p2.x),                        // src x
                  min(p1.y,p2.y),                      // src y
                  abs(p1.x-p2.x),                // src width
                  abs(p1.y-p2.y),            // src height
                  GetBitsAddress(),         // bits
                  GetBitmapInfoAddress(),   // BITMAPINFO
                  DIB_RGB_COLORS,           // options
                  SRCCOPY);                 // rop

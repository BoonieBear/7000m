...
// Construction
public:
	void ExpainPskAndFsk(BYTE* databuf, int len);
	void PackageUWADataInPsk(unsigned char *buf, int len);
	void PackagePicTureIn512x512(unsigned char *buf, int len);
	void TestPicturePackageAndParse();
...
//	void ShowLog(CString str);
	...
	BYTE CompressValue[IMAGESIZE512X512];//存放压缩的图像数据
	BYTE *RecDataBuf;//接收到的帧数据
	int  RecFrameNo; //已接收到的帧号
	int ErrIndex;//校验出错的帧号

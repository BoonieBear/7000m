...
void CImageSelect::OnOK() 
{
	...
	NewImage.Resample(256,256,2,NULL);
...
// 	{
		...
		image.Draw(pdc->GetSafeHdc(),(500-(int)(image.GetWidth()*zoom))/2,(300-(int)(image.GetHeight()*zoom))/2,
		...
		NewImage.Draw(pdc->GetSafeHdc(),510,10,256,256,0,TRUE);
// 	}
// 	showed = true;
...
void CImageSelect::GetMaxZoomFactor(DWORD Height, DWORD Width)
{
	ASSERT(Height > 0);
	ASSERT(Width > 0);
	if ((Height<=256)&&(Width<=256))//图片小于256*256大小不变
	{
		ZoomFactor = 1;
	}
	else
...
void CImageSelect::GetSelectZoomFactor(DWORD Height, DWORD Width)
{
	...
	{
		SelectZoomFactor = min(256.0/Height,256.0/Width);
	}
